import { notFound } from "next/navigation";
import { loadMasterDb, filterByCuisine } from "@/lib/data";
import { RestaurantCard } from "@/components/RestaurantCard";
import { CUISINE_ICONS, CUISINE_LABELS } from "@/lib/types";
import { BreadcrumbJsonLd, CollectionPageJsonLd } from "@/components/JsonLd";
import { AdSlot } from "@/components/AffiliateSlot";
import { StatsBar } from "@/components/StatsBar";
import { sortWithSponsored } from "@/lib/sponsored";
import { cuisineLabel, CUISINE_HUB_COPY } from "@/lib/hubI18n";
import type { Metadata } from "next";

const VALID = new Set(Object.keys(CUISINE_LABELS));

export function cuisineStaticParams() {
  return Array.from(VALID).map((cuisine) => ({ cuisine }));
}

export async function cuisineMetadata(
  { params }: { params: Promise<{ cuisine: string }> },
  locale: "th" | "ko"
): Promise<Metadata> {
  const { cuisine } = await params;
  const label = cuisineLabel(cuisine, locale);
  const db = await loadMasterDb();
  const list = filterByCuisine(db.restaurants, cuisine);
  const count = list.length;

  const title = locale === "th"
    ? `${label}${count} ร้านในกรุงเทพฯ — รีวิวจริง ไม่ใช่กระแส SNS`
    : `방콕 ${label} 맛집 ${count}곳 — 진짜 리뷰, SNS 과대광고 아님`;
  const description = locale === "th"
    ? `ร้าน${label} ${count} แห่งที่ผ่านการตรวจสอบในกรุงเทพฯ และพัทยา วิเคราะห์จากรีวิว Google จริง ไม่มีการจัดอันดับจากอินฟลูเอนเซอร์`
    : `방콕·파타야 검증된 ${label} 맛집 ${count}곳을 실제 구글 리뷰로 분석. 인플루언서 협찬 랭킹이 아닙니다.`;

  return {
    title,
    description,
    alternates: {
      canonical: `/${locale}/c/${cuisine}`,
      languages: {
        en: `/c/${cuisine}`,
        th: `/th/c/${cuisine}`,
        ko: `/ko/c/${cuisine}`,
        "x-default": `/c/${cuisine}`,
      },
    },
    openGraph: {
      title,
      description,
      url: `/${locale}/c/${cuisine}`,
      siteName: "SNS Stopper",
      locale: locale === "th" ? "th_TH" : "ko_KR",
    },
  };
}

export async function CuisineHubContent(
  { params }: { params: Promise<{ cuisine: string }> },
  locale: "th" | "ko"
) {
  const { cuisine } = await params;
  if (!VALID.has(cuisine)) notFound();

  const db = await loadMasterDb();
  const filtered = sortWithSponsored(filterByCuisine(db.restaurants, cuisine));
  const label = cuisineLabel(cuisine, locale);
  const icon = CUISINE_ICONS[cuisine] ?? "🍴";
  const copy = CUISINE_HUB_COPY[locale];

  const byDistrict = new Map<string, number>();
  for (const r of filtered) {
    if (!r.district) continue;
    byDistrict.set(r.district, (byDistrict.get(r.district) ?? 0) + 1);
  }
  const districts = Array.from(byDistrict.entries())
    .filter(([, n]) => n >= 3)
    .sort((a, b) => b[1] - a[1]);

  const totalReviews = filtered.reduce((s, r) => s + r.total_reviews, 0);
  const withScraped = filtered.filter((r) => r.scraped_review_count > 0).length;

  return (
    <div lang={locale}>
      <StatsBar
        generatedAt={db.generated_at}
        totalClinics={filtered.length}
        totalReviews={totalReviews}
        withScraped={withScraped}
        entityLabel={locale === "th" ? "ร้าน" : "맛집"}
      />
      <div className="max-w-5xl mx-auto px-4 py-8">
        <nav className="text-sm text-[var(--muted)] mb-4">
          <a href={`/${locale}`} className="hover:text-[var(--fg)]">{copy.home}</a>
          <span className="mx-2">›</span>
          <span>{label}</span>
        </nav>
        <h1 className="text-3xl md:text-4xl font-bold tracking-tight mb-2 flex items-center gap-3">
          <span>{icon}</span>
          {copy.heading(label)}
        </h1>
        <p className="text-[var(--muted)] mb-8">{copy.intro(filtered.length)}</p>

        {districts.length > 0 && (
          <section className="mb-10">
            <h2 className="text-sm font-semibold uppercase tracking-wide text-[var(--muted)] mb-3">
              {copy.byDistrict(label)}
            </h2>
            <div className="flex flex-wrap gap-2">
              {districts.map(([d, n]) => (
                <a
                  key={d}
                  href={`/${locale}/d/${d.toLowerCase().replace(/\s+/g, "-")}`}
                  className="px-3 py-1.5 rounded-full border border-[var(--border)] text-sm bg-white hover:border-[var(--accent)] hover:text-[var(--accent)] transition"
                >
                  {copy.districtPill(label, d)} <span className="text-[var(--muted)]">{n}</span>
                </a>
              ))}
            </div>
          </section>
        )}

        <section>
          <h2 className="text-xl font-bold mb-4">{copy.top(Math.min(filtered.length, 36))}</h2>
          <div className="grid gap-3">
            {filtered.slice(0, 10).map((r, i) => (
              <RestaurantCard key={r.id} r={r} rank={i + 1} />
            ))}
          </div>

          <AdSlot slot="cuisine-mid" />

          <div className="grid gap-3 mt-3">
            {filtered.slice(10, 36).map((r, i) => (
              <RestaurantCard key={r.id} r={r} rank={i + 11} />
            ))}
          </div>

          {filtered.length > 36 && (
            <p className="mt-6 text-sm text-[var(--muted)]">{copy.more(filtered.length - 36)}</p>
          )}
        </section>

        <BreadcrumbJsonLd items={[
          { name: "Home", url: `/${locale}` },
          { name: label, url: `/${locale}/c/${cuisine}` },
        ]} />
        <CollectionPageJsonLd
          name={`Top ${CUISINE_LABELS[cuisine] ?? cuisine} Restaurants in Bangkok`}
          description={`${filtered.length} verified ${(CUISINE_LABELS[cuisine] ?? cuisine).toLowerCase()} restaurants ranked by Trust Score from Google review analysis.`}
          url={`/${locale}/c/${cuisine}`}
          items={filtered}
        />
      </div>
    </div>
  );
}
