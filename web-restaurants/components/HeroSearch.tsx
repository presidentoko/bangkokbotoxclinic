...
export function HeroSearch({
  ...
  const hasTitle = !!hero;
  if (!hasTitle) {
    // Embedded mode — used inside larger hero sections (mega home page).
    return (
          ...
          <div className="mt-4 flex items-center justify-center gap-2 flex-wrap text-xs">
            ...
            {popularSearches.map((p) => (
              <a
                key={p.href}
                href={p.href}
                className="px-3 py-1 rounded-full bg-white border border-[var(--border)] hover:border-orange-400 hover:bg-orange-50 text-[var(--fg)] transition"
              >
                {p.label}
              </a>
            ))}
