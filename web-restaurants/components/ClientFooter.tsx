    ...
    <footer className="border-t border-[var(--border)] mt-16 bg-[var(--card)]">
      <div className="max-w-5xl mx-auto px-4 py-8 text-sm text-[var(--muted)]">
        ...
        <p className="text-xs text-[var(--muted)] mb-5 max-w-xl">
          {tr(s.desc, locale)}{" "}
          <Link href="/famous-vs-good" className="text-[var(--accent)] hover:underline font-medium">
            ...
            {topCuisines.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1.5">
                  {topCuisines.map((cat) => (
                    <Link key={cat} href={localizedHubHref(`/c/${cat}`, locale)} className="hover:text-[var(--fg)]">
        ...
        <div className="flex flex-wrap gap-x-8 gap-y-3 mb-4">
          <Link href="/famous-vs-good" className="hover:text-[var(--fg)] font-medium">{tr(c.snsCheck, locale)}</Link>
          <Link href="/about" className="hover:text-[var(--fg)]">{tr(c.about, locale)}</Link>
          <Link href="/contact" className="hover:text-[var(--fg)]">{tr(c.contact, locale)}</Link>
          <Link href="/for-restaurants" className="hover:text-[var(--fg)]">{tr(c.forRestaurants, locale)}</Link>
        ...
        <div className="flex flex-wrap gap-x-6 gap-y-2 mb-4 text-xs">
          <Link href="/" hrefLang="en" className="hover:text-[var(--fg)]">English</Link>
          <Link href="/th" hrefLang="th" className="hover:text-[var(--fg)]">ไทย</Link>
          <Link href="/ko" hrefLang="ko" className="hover:text-[var(--fg)]">한국어</Link>
        </div>
