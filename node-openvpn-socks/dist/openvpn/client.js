...
const static_key_1 = require("./static_key");
/**
...
class OpenVPNClient extends events_1.EventEmitter {
    constructor(config) {
        super();
        this.config = config;
        // TCP Framing Buffer
        ...
        this.iv_proto = (1 << 1) | (1 << 2) | (1 << 4) | (1 << 6) | (1 << 7) | (1 << 8) | (1 << 9); // 982
        this.serverKeyMaterialReceived = false;
        this.tlsRecvBuffer = Buffer.alloc(0);
        this.keyBlockIsEkm = false;
        this.controlChannel = new control_1.ControlChannel();
        this.virtualTransport = new transport_1.VirtualTransport(this.controlChannel);
        this.setupSocket();
        this.setupControlChannel();
    }
    setupSocket() {
        const remote = this.config.remote[0];
        const proto = this.config.proto;
        const isTcp = proto.includes('tcp');
        // TLS Auth Setup
        if (this.config.tlsAuth && this.config.tlsAuth.key) {
            try {
                console.log('Configuring TLS-Auth...');
                const keyBuf = static_key_1.OpenVPNStaticKey.parse(this.config.tlsAuth.key);
                const direction = this.config.tlsAuth.direction;
                const { sendKey, recvKey } = static_key_1.OpenVPNStaticKey.getHmacKeys(keyBuf, direction);
    ...
    processIncomingPacket(msg) {
        ...
        if (opcode === packet_1.Opcode.P_DATA_V1 || opcode === packet_1.Opcode.P_DATA_V2) {
    ...
    setupControlChannel() {
        this.controlChannel.on('send', (buffer) => {
    ...
    connect() {
        ...
        this.tlsSocket = tls.connect(options, () => {
            ...
            console.log(`TLS Handshake established! proto=${proto} cipher=${cipher?.name} kx=${cipher?.standardName}`);
            this.onTlsSecure();
        });
    ...
    onTlsSecure() {
        ...
        const peerInfoStr = `IV_VER=2.6.16\n` +
            `IV_PLAT=linux\n` +
            `IV_TCPNL=1\n` +
            `IV_MTU=1600\n` +
            `IV_NCP=2\n` +
    ...
    processTlsData() {
        if (!this.serverKeyMaterialReceived) {
            ...
            if (this.tlsRecvBuffer.length < minNoPre)
                return;
            const tryParse = (hasPreMaster) => {
                let off = 4;
                if (this.tlsRecvBuffer[off] !== 0x02)
                    return false;
                off += 1;
                if (hasPreMaster)
                    off += 48;
                off += 32 + 32; // random1 + random2
                if (this.tlsRecvBuffer.length < off + 2)
                    return false;
                const optsLen = this.tlsRecvBuffer.readUInt16BE(off);
                off += 2;
                if (optsLen > 2048 || optsLen < 4)
                    return false;
                if (this.tlsRecvBuffer.length < off + optsLen)
                    return false;
            ...
            if (hasPreMaster)
                off += 48;
            const serverRandom1 = this.tlsRecvBuffer.slice(off, off + 32);
            off += 32;
            const serverRandom2 = this.tlsRecvBuffer.slice(off, off + 32);
            off += 32;
            const optsLen = this.tlsRecvBuffer.readUInt16BE(off);
            off += 2;
        ...
        while (this.tlsRecvBuffer.length > 0) {
            ...
            if (this.tlsRecvBuffer.length === 0)
                break;
            ...
            if (msg.startsWith('PUSH_REPLY'))
                this.applyPushReply(msg);
            this.emit('control-message', msg);
        }
    }
    deriveKeys(serverRandom1, serverRandom2) {
        ...
        this.rebuildSecurityContext((this.config.cipher || 'AES-128-CBC').toUpperCase(), (this.config.auth || 'SHA1').toUpperCase(), 0, true);
    }
    /**
     ...
     */
    computeKeyBlock(useEkm) {
        if (!this.clientPreMaster || !this.clientRandom1 || !this.clientRandom2)
            return;
        if (!this.derivationState)
            return;
        ...
        if (!useEkm) {
            ...
            const master = crypto_1.OpenVPNCrypto.prf(this.clientPreMaster, 'OpenVPN master secret', masterSeed, 48);
            const expSeed = Buffer.concat([this.clientRandom2, serverRandom2, localSid, remoteSid]);
            this.keyBlock = crypto_1.OpenVPNCrypto.prf(master, 'OpenVPN key expansion', expSeed, 256);
     ...
     */
    rebuildSecurityContext(cipherName, authName, peerId, resetPacketId) {
        if (!this.keyBlock)
            return;
        ...
        if (authName === 'SHA256')
            hmacKeyLen = 32;
        else if (authName === 'SHA384')
            hmacKeyLen = 48;
        else if (authName === 'SHA512')
            hmacKeyLen = 64;
        else if (authName === 'MD5')
            hmacKeyLen = 16;
    ...
    sendDataPacket(ipPacket) {
        ...
        if (isGcm) {
            ...
            header[0] = (packet_1.Opcode.P_DATA_V2 << 3) | keyId;
            header.writeUIntBE(ctx.peerId & 0xffffff, 1, 3);
            const pidBuf = Buffer.alloc(4);
            pidBuf.writeUInt32BE(ctx.packetId, 0);
            const aad = Buffer.concat([header, pidBuf]);
            const { ciphertext, tag } = crypto_1.OpenVPNCrypto.encryptGcm(ipPacket, ctx.encCipherKey, ctx.encIv, ctx.packetId, aad);
            ...
            const header = useV2
                ? (() => { const b = Buffer.alloc(4); b[0] = (packet_1.Opcode.P_DATA_V2 << 3) | keyId; b.writeUIntBE(ctx.peerId & 0xffffff, 1, 3); return b; })()
                : Buffer.from([(packet_1.Opcode.P_DATA_V1 << 3) | keyId]);
            const iv = crypto.randomBytes(16);
            const pidBuf = Buffer.alloc(4);
            pidBuf.writeUInt32BE(ctx.packetId, 0);
            const plaintext = Buffer.concat([pidBuf, ipPacket]);
            const ciphertext = crypto_1.OpenVPNCrypto.encryptCbc(plaintext, ctx.encCipherKey, iv, cipherName);
            const hmac = crypto_1.OpenVPNCrypto.calculateHmac(Buffer.concat([iv, ciphertext]), ctx.encHmacKey, ctx.auth);
    ...
    sendRawPacket(packet) {
        if (this.tcpSocket) {
            ...
            this.udpSocket.send(packet, remote.port, remote.host, (err) => {
                if (err)
                    console.error(`UDP TX failed (${remote.host}:${remote.port}):`, err.message);
            });
            console.log(`UDP TX: Packet len=${packet.length} Hex=${packet.toString('hex')}`);
        }
    }
    handleDataChannelPacket(buffer) {
        if (!this.securityContext)
            return;
        const ctx = this.securityContext;
        ...
        if (isGcm) {
            ...
            let headerLen;
            if (opcode === packet_1.Opcode.P_DATA_V2)
                headerLen = 4;
            else if (opcode === packet_1.Opcode.P_DATA_V1)
                headerLen = 1;
            else
                return;
            if (buffer.length < headerLen + 4 + 16)
                return;
            ...
            try {
                const plaintext = crypto_1.OpenVPNCrypto.decryptGcm(ciphertext, ctx.decCipherKey, ctx.decIv, packetId, tag, aad);
                this.emitIpPacket(plaintext);
            }
            catch (e) {
                console.error(`Decrypt GCM failed (pid=${packetId}): ${e.message}`);
            }
        }
        else {
            const opcode = buffer[0] >> 3;
            const headerLen = opcode === packet_1.Opcode.P_DATA_V2 ? 4 : 1;
            let hmacLen = 20;
            if (ctx.auth === 'SHA256')
                hmacLen = 32;
            else if (ctx.auth === 'SHA384')
                hmacLen = 48;
            else if (ctx.auth === 'SHA512')
                hmacLen = 64;
            else if (ctx.auth === 'MD5')
                hmacLen = 16;
            if (buffer.length < headerLen + hmacLen + 16)
                return;
            ...
            const expectedHmac = crypto_1.OpenVPNCrypto.calculateHmac(Buffer.concat([iv, ciphertext]), ctx.decHmacKey, ctx.auth);
            if (!expectedHmac.equals(rxHmac)) {
                console.error(`Data HMAC mismatch. got=${rxHmac.toString('hex')} expected=${expectedHmac.toString('hex')}`);
                return;
            }
            try {
                const plaintext = crypto_1.OpenVPNCrypto.decryptCbc(ciphertext, ctx.decCipherKey, iv, ctx.cipher);
                if (plaintext.length > 4)
                    this.emitIpPacket(plaintext.slice(4));
            }
            catch (e) {
                console.error('Decrypt CBC failed', e.message);
            }
        }
    }
    emitIpPacket(ipPacket) {
        if (ipPacket.length === 0)
            return;
...
exports.OpenVPNClient = OpenVPNClient;
OpenVPNClient.PING_MAGIC = Buffer.from('2a187bf3641eb4cb07ed2d0a981fc748', 'hex');
