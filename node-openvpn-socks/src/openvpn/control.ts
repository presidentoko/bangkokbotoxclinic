...
import { OpenVPNPacket, Opcode, PacketParser } from './packet';
import * as crypto from 'crypto';



export class ControlChannel extends EventEmitter {
    private localSessionId: Buffer;
    private remoteSessionId: Buffer | undefined;
    private packetId: number = 0; // Next packet ID to send
    private expectedPacketId: number = 0; // Next packet ID expected from remote

    // Simplistic retransmission queue: Map<PacketID, {packet: Buffer, timestamp: number}>
    private sentPackets: Map<number, { buffer: Buffer, retryCount: number, lastSent: number }> = new Map();

    // State Machine
    private state: 'INITIAL' | 'CONNECTING' | 'READY' = 'INITIAL';
    private outgoingQueue: Buffer[] = [];

    // TLS Auth
    private tlsAuthSendKey?: Buffer;
    private tlsAuthRecvKey?: Buffer;
    ...
    private tlsAuthDigest: string = 'sha1';
    private tlsAuthHmacLen: number = 20;
    ...
    constructor() {
        ...
        // Random session ID
        for (let i = 0; i < 8; i++) this.localSessionId[i] = Math.floor(Math.random() * 256);
    }

    public setTlsAuth(sendKey: Buffer, recvKey: Buffer, authDigest: string = 'SHA1') {
    ...
    public getLocalSessionId(): Buffer { return this.localSessionId; }
    public getRemoteSessionId(): Buffer | undefined { return this.remoteSessionId; }
    ...
    public handlePacket(buffer: Buffer) {
        try {
            ...
            const packet = PacketParser.parse(payload);
            ...
            if (packet.packetIdArray && packet.packetIdArray.length > 0) {
                ...
                packet.packetIdArray.forEach((ackId: number) => {
            ...
            const isControl = (
                packet.opcode === Opcode.P_CONTROL_V1 ||
                packet.opcode === Opcode.P_CONTROL_HARD_RESET_SERVER_V2
            );
            ...
            if (isControl) {
                ...
                if (packet.opcode === Opcode.P_CONTROL_HARD_RESET_SERVER_V2) {
                ...
                if (packet.messageId !== undefined) {
                    ...
                    if (serverSid) this.sendAck(packet.messageId, serverSid);

                    if (packet.messageId === this.expectedPacketId) {
                        this.expectedPacketId++;
                        // Only emit P_CONTROL_V1 payloads (TLS Data)
                        if (packet.opcode === Opcode.P_CONTROL_V1 && packet.payload && packet.payload.length > 0) {
    ...
    public send(data: Buffer) {
        ...
        const packet = this.createPacket(Opcode.P_CONTROL_V1, pid, this.localSessionId, data);
    ...
    public start() {
        ...
        const pid = this.packetId++;
        const packet = this.createPacket(Opcode.P_CONTROL_HARD_RESET_CLIENT_V2, pid, this.localSessionId, Buffer.alloc(0));
        this.sendPacketReliable(pid, packet);
        ...
        setTimeout(() => {
            if (this.state === 'CONNECTING' && this.sentPackets.has(0)) {
                ...
                const v3packet = this.createPacket(Opcode.P_CONTROL_HARD_RESET_CLIENT_V3, v3pid, this.localSessionId, Buffer.alloc(0));
    ...
    private createPacket(opcode: Opcode, packetId: number, sessionId: Buffer, payload: Buffer): Buffer {
        const header = Buffer.alloc(1);
        header.writeUInt8((opcode << 3), 0);
    ...
    private sendPacketReliable(pid: number, buffer: Buffer) {
    ...
    private emitSend(packet: Buffer) {
