...
export default function HospitalCard({ hospital, distanceKm }: Props) {
  return (
    <Link href={`/hospital/${hospital.id}`} className="block bg-white border rounded-xl p-4 hover:shadow-md transition-shadow">
      ...
      <div className="flex items-center gap-3 text-xs text-gray-500">
        {distanceKm != null && (
          <span className="text-green-600 font-medium">📍 {distanceKm < 1 ? `${Math.round(distanceKm * 1000)} ม.` : `${distanceKm.toFixed(1)} กม.`}</span>
        )}
