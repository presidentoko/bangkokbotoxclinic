...
// 2026-08-14 감사: revalidate 가 없어 이 라우트가 빌드 시점에 영구 고정되고,
// 그 응답이 populateCache 로 Worker 배포 자산에 읽기 전용으로 구워진다.
// Cloudflare Purge Everything 은 엣지 캐시만 지우고 이 계층엔 안 닿는다 —
// 그래서 noindex 게이트를 고쳐 재배포해도 사이트맵 응답이 갱신 안 됐다
// (페이지 자체의 noindex 메타는 정상 반영됨, 사이트맵만 정체). 24h 로 재검증되게 한다.
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const items: MetadataRoute.Sitemap = [
    ...
    // 2026-08-20: 아래 8개는 색인 가능한 공개 페이지인데 사이트맵에 없었다.
    // 제외: /admin·/dashboard·/onboarding·/pay·/saved(비공개), /compare(robots Disallow).
  ...
  // 조건을 바꿀 땐 반드시 해당 page.tsx 와 같이 바꿀 것. (web/ 원본과 동기)
  for (const s of HUB_SERVICES) {
    // app/c/[service]/page.tsx — scoped 카테고리 5개 미만이면 noindex
    ...
    // 2026-08-20: /th/c/[service] 는 2026-08-06 에 생겼는데 사이트맵에 한 번도
    // 오르지 않아 완전한 고아였다. 이 사이트의 GSC 상위 쿼리는 전부 태국어인데
    // 정작 그 착지 페이지들이 제출조차 안 된 상태였다.
  ...
  for (const c of BEST_FOR) {
    ...
    // 2026-08-20: 양쪽 다 합산으로 셌는데 본문 렌더(applySiteFilter)와 어긋나
    // 거의 빈 /best/ 페이지를 색인 요청하고 있었다. 페이지와 같이 scoped 로 옮김.
  ...
  for (const d of districts) {
    ...
    // 에선 5개 미만인 지역이 통과된다 — 그게 /d/ 모순 건들의 원인이었다.
