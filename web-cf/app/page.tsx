...
// ── 홈 렌더 개수 (2026-08-13 성능 조사 결과: 건드리지 않기로 함) ─────────────
// 이 두 숫자가 홈 응답 크기를 지배한다. 실측:
//   HTML 743KB · RSC 플라이트 486KB(65%) · ClinicCardCompact 76행 · 클리닉 110곳 직렬화
// www 는 Cloudflare Worker(web-cf)가 서빙하는데 엣지 캐시가 안 걸려 있어 모든 요청이
// Worker 를 돌리고, 그러다 CPU 한계를 넘어 `error 1102` → 503 이 난다.
// 동시 8요청 2.4%, 40요청 5%, **초당 0.5요청에서도** 재현. GSC 의 5xx 655건 +
// "Discovered - currently not indexed" 1,061건이 이것이다.
//
// 한 번 6/30 으로 줄여 빌드해 실측했다: HTML 743→601KB(-19%), 플라이트 486→401KB.
// **그런데 그걸로는 1102 가 사라지지 않는다** — 요청당 Worker 가 도는 건 그대로라
// 601KB 든 743KB 든 결과가 같다. 반면 대가는 확실했다: 홈 내부링크 76→38, 이 사이트는
// 3사이트 중 유일하게 잘 되는 곳(클릭 1,690·평균순위 9.8)이다.
// → 해결도 안 되는 문제 때문에 잘 되는 페이지를 깎는 거래라 되돌렸다.
//
// 진짜 해법은 **Cloudflare Cache Rules(엣지 캐시)** 하나다. 그게 걸리기 전엔
// 이 숫자를 만지지 말 것. 걸린 뒤엔 만질 이유가 없어진다.
const CITY_ROW_LIMIT = 12;   // 도시당 카드
const RUNNER_UP_END = 50;    // top[10..50)
            ...
            <div className="mb-12 space-y-10">
              {topCities.map(([city, clinics]) => (
                <CityRow
                  ...
                  lang={lang}
                >
                  {/* 2026-08-13 성능: 도시당 12 → 6. 홈 한 장에 ClinicCardCompact 가
                      76행(도시 3×12 + runner-up 40) 렌더돼 RSC 플라이트가 486KB 였다.
                      Cloudflare Worker 가 매 요청 이걸 만들다 CPU 한계(error 1102)로
                      503 을 뱉었다 — 초당 0.5요청에도 재현됐다. 실측 기반 축소다. */}
                  {clinics.slice(0, CITY_ROW_LIMIT).map((c, idx) => (
            ...
            <div className="mt-8">
              <div className="flex items-baseline justify-between mb-3">
                <h3 className="text-sm font-bold uppercase tracking-widest text-[var(--muted)]">
                  {/* `#11 – #{n}` 의 {n} 은 개수가 아니라 **마지막 순위**다.
                      실제로 그리는 마지막 순위와 맞춘다 (RUNNER_UP_END 를 줄이면 자동 반영). */}
                  {t.runnerUp.replace("{n}", String(Math.min(top.length, RUNNER_UP_END)))}
                </h3>
              ...
              <div className="grid gap-1.5">
                {top.slice(10, RUNNER_UP_END).map((c, i) => (
