...
export default function TrustScoreGame({ clinics }: { clinics: Clinic[] }) {
  // Eligible: trust_score and rating defined
  const pool = clinics.filter((c) => c.trust_score >= 30 && c.rating !== null && (c.total_reviews ?? 0) >= 10);
  ...
  return (
    <section className="rounded-[2rem] border-2 bg-gradient-to-br from-violet-50 via-purple-50 to-pink-50 dark:from-violet-950/30 dark:to-pink-950/30 p-6 sm:p-8" style={{ borderColor: "var(--border)" }}>
      ...
      {/* Clinic card */}
      <div className="rounded-2xl bg-white dark:bg-navy-900/30 border p-5 mb-4" style={{ borderColor: "var(--border)" }}>
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div className="min-w-0">
            <div className="font-display text-xl font-bold tracking-tighter-display truncate">{clinic.name}</div>
            <div className="text-xs muted mt-0.5">{clinic.city_label || clinic.district || ""} · {clinic.primary_type || ""}</div>
          </div>
          <div className="text-right shrink-0">
            <div className="text-2xl font-black tabular-nums text-yellow-700">★ {(clinic.rating ?? 0).toFixed(1)}</div>
            <div className="text-[10px] muted">{((clinic.total_reviews ?? 0) ?? 0).toLocaleString()} reviews</div>
          </div>
        </div>
        <div className="mt-3 grid grid-cols-3 gap-2 text-center text-[10px]">
          <div className="rounded bg-blue-50 dark:bg-blue-950/30 p-2">
            <div className="font-bold tabular-nums">{clinic.total_reviews ?? 0}</div>
            <div className="muted">Google</div>
          </div>
          <div className="rounded bg-purple-50 dark:bg-purple-950/30 p-2">
            <div className="font-bold tabular-nums">{clinic.scraped_review_count ?? 0}</div>
            <div className="muted">Deep-scraped</div>
          </div>
          <div className="rounded bg-red-50 dark:bg-red-950/30 p-2">
            <div className="font-bold tabular-nums">{clinic.local_guide_count ?? 0}</div>
            <div className="muted">Local Guides</div>
          </div>
        </div>
      </div>
