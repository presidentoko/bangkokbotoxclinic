...
import { POSTS_TH, findPostTh } from "@/lib/posts_th";
...
export async function generateStaticParams() {
  return POSTS_TH.map((p) => ({ slug: p.slug }));
}
...
export async function generateMetadata(
  ...
  const p = findPostTh(slug);
  if (!p) return { title: "ไม่พบบทความ" };
  return {
    title: p.metaTitle,
    description: p.metaDescription,
    alternates: { canonical: `/th/blog/${slug}`, languages: { "th-TH": `/th/blog/${slug}`, "x-default": `/th/blog/${slug}` } },
    openGraph: {
      title: p.metaTitle,
      description: p.metaDescription,
      type: "article",
      url: `${SITE}/th/blog/${slug}`,
      locale: "th_TH",
...
function articleJsonLd(p: ReturnType<typeof findPostTh> & object) {
  return (
    <script
      ...
      dangerouslySetInnerHTML={{
        __html: JSON.stringify({
          ...
          mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE}/th/blog/${p.slug}` },
          inLanguage: "th",
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      <nav className="text-sm text-[var(--muted)] mb-4">
        ...
        <a href="/th/blog" className="hover:text-[var(--fg)]">บล็อก</a>
        <span className="mx-2">›</span>
      ...
      <section className="mt-12 bg-emerald-50 border border-emerald-200 rounded-2xl p-6 md:p-8">
        <h2 className="text-xl font-bold text-emerald-900 mb-2">ค้นหาซัพพลายเออร์โดยตรง</h2>
        <p className="text-sm text-emerald-800 mb-5">
          ซัพพลายเออร์ B2B 3,300+ ราย ตรวจสอบกับ DBD แล้ว — ติดต่อตรง ไม่ผ่านนายหน้า ข้อมูลทุนจดทะเบียน วันก่อตั้ง รหัส TSIC
        </p>
        <div className="flex flex-wrap gap-3">
          ...
          <a href="/th/guide" className="px-4 py-2 bg-white border border-emerald-300 text-emerald-900 text-sm font-bold rounded-lg hover:bg-emerald-100 transition">
            คู่มือผู้ซื้อ →
          </a>
        </div>
      </section>
        ...
        <section className="mt-12 pt-8 border-t border-[var(--border)]">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {p.related.map((relSlug) => {
              const rel = findPostTh(relSlug);
              if (!rel) return null;
              return (
                <a
                  key={relSlug}
                  href={`/th/blog/${relSlug}`}
