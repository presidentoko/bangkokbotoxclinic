...
- **개발 격리:** git worktree `worktree-cosmetics-aeo` (기존 repo의 watchdog 자동 커밋/체크아웃과 충돌 방지)
- **상태:** 설계 승인 완료 → 구현 계획(writing-plans)으로 전환 예정
...
## 6. VPN 할당 (충돌 방지) — 자동 폴백 방식

- 현재 기존 프로젝트: `nordvpn_runner --ports 8 --base-port 2080` → **2080–2087 전부 점유**
- 기존 8터널이 정상 작동 중이므로 NordVPN 계정 동시 접속 한도는 **최소 8 이상**(현 표준 10일 가능성 높음, 단 8/10 미확정).
- **설계 방침 — 한도를 몰라도 동작:** 화장품 runner는
  1. **1순위: 전용 2터널 시도** — 포트 2090–2091, 별도 `nordvpn_runner` 인스턴스로 기동.
  2. **연결 실패(한도 초과) 시 자동 시분할 폴백** — 클리닉 그리드가 한가한 시간대에 기존 풀 일부(2086–2087)를 빌려 배치 실행, watchdog `.disabled` 로직으로 충돌 회피.
- 한도가 10이면 전용 2터널 상설(8+2=10), 8이면 자동으로 시분할로 떨어짐.
- 화장품은 유한 배치 작업이라 2터널로 충분 (24시간 무한 스윕 불필요).
- (선택) NordVPN 대시보드의 "X devices simultaneously" 표기로 한도를 직접 확인하면 폴백 분기 없이 확정 가능.

## 7. 디렉토리·배포 구조

- 신규 코드: 기존 repo 내 `cosmetics/` 모듈로 두어 VPN runner·watchdog·Pantip 스크래퍼 등 공용 인프라 재사용. (개발은 worktree에서 진행 후 머지)
...
- 여드름·미백 외 고민(주름/민감성 등)은 검증 후 확장.
