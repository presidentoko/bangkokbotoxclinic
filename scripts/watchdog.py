...
def build_services() -> list[Service]:
    ...
    bangkok_env = {
        ...
        # 2026-09-02: 2→1. 총 5워커가 이 머신에 빠듯했다 — 09:00 에 여유가 0.8GB 까지
        # 떨어져 이 서비스가 pause 됐다(1분 뒤 2.1GB 회복). 밤새 705건을 처리했지만
        # 한 서비스가 주기적으로 양보하는 진동이 있었다. 총 4워커면 그 완충이 생긴다.
        "N_WORKERS": "3",
        # 2026-09-11: 5→3. 사용자 지시(C안). 5워커는 이 머신에서
        # 여유 RAM 을 0.6GB 대로 떨어뜨렸고, 그러면 크롬이 리뷰를 지연
        # 로딩하지 못해 스크롤이 10개에서 멈춘다(9/11 새벽 84%가
        # DOM 1~10개). 수집 평균이 31개→13.6개, 시간당 완료가
        # 34.5→12.7 로 떨어졌다 — 병렬도를 올려 전체가 느려졌다.
        # 터널은 2083-2085 를 쓰고 2086-2087 은 비워둔다 — claim_live_port
        # 가 배정 터널이 죽었을 때 빌려 쓸 여유분이 된다.
        # (이전 주석) 2026-09-08: 3→5. 필요 터널이 3→5개라
        # PROXY_PORT_BASE 를 2085→2083 으로 내려 2083-2087 을 쓴다
        # (base + N_WORKERS - 1 이 2087 을 넘으면 _validate_proxy_ports 가
        # 부팅을 거부한다). 지금 터널을 쓰는 서비스는 이것과
        # spa_grid_koh_samui(2080) 뿐이라 2081-2082 는 비워둔다 —
        # bangkok_clinics_review 가 깨어나면 그쪽을 쓴다.
        # 원래 배정은 2080-2081 이지만 그 두 슬롯이 실질적으로 죽어 있다.
        # nordvpn_runner 로그에서 최근 구간 기준 spawn/READY 가
        #   2080: 533/184   2081: 515/158   2082: 505/169
        #   2083: 60/25  2084: 41/20  2085: 28/20  2086: 34/20  2087: 49/20
        # 앞 3슬롯만 10배 넘게 재접속을 반복한다. 서버를 계속 바꿔가며 실패하니
        # 특정 서버 문제가 아니라 계정 동시접속 한도(8개 요청)를 넘겨 남는
        # 슬롯이 영구히 밀리는 것으로 보인다 — 근본 해결은 --ports 를 한도에
        # 맞춰 줄이는 것.
    ...
    bangkok_clinics_env = {
        ...
        # 2026-09-02: 2→4. 오전 성공률이 기준선을 재현했다(10시 60% vs 08-31 58%).
        # 하루 곡선도 안정적이다: 05시 71% · 07시 74% · 10시 60%, 09-01 오후의
        # 2~9% 구간은 사라졌다. 회복 요인은 재스캔 해방 — 큐 1,000건 중 874건이
        # 재스캔이라 죽은 잔여물이 밀려났다. 이제 처리량을 늘릴 차례다.
        # 2026-09-03: 4→2. 09-02 에 4로 올린 건 클리닉 큐가 꽉 찼을 때의 판단이었고,
        # 그때는 식당 두 개가 워커 1개씩이라 총 6이었다. 지금은 방콕 식당이 2워커라
        # 총 7이 되어 RAM 여유가 0.35GB(pause 0.9 아래)로 떨어지고 식당이 밀려났다.
        # 클리닉 큐는 671건 — 2워커로도 며칠 안에 소화한다. 공존이 우선이다.
        "N_WORKERS": "2",
        "PROXY_PORT_BASE": "2080",
        "SEARCH_TAG": "en",
        "CITY_LAT": "13.7462890",
