...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: isEn
      ? `Chanel Mini vs Small Classic Flap Thailand ${PRICE_YEAR} | ChicPreowned`
      : `Chanel Mini vs Small Classic Flap ในไทย ${PRICE_YEAR} | ChicPreowned`,
    description: isEn
      ? `Chanel Mini Flap vs Small Classic Flap for Thai buyers — dimensions, what fits, THB price difference, which size to buy pre-owned in Bangkok ${PRICE_YEAR}.`
      : `Chanel Mini Flap vs Small Classic Flap สำหรับผู้ซื้อชาวไทย — ขนาด ใส่อะไรได้บ้าง ราคาต่างกันเท่าไหร่ ขนาดไหนควรซื้อมือสองในกรุงเทพ ${PRICE_YEAR}`,
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      <h1 className="text-3xl font-bold text-gray-900 mb-4">
        {isEn ? `Chanel Mini Flap vs Small Classic Flap ${PRICE_YEAR}` : `Chanel Mini Flap vs Small Classic Flap ${PRICE_YEAR}`}
      </h1>
