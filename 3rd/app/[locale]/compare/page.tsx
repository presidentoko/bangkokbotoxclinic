...
import { getComparePages, humanizeSlug } from '@/lib/site-pages'
...
export default async function CompareHubPage({ params }: Props) {
  ...
  const compares = getComparePages()
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      <div className="flex gap-4 text-sm mb-8 border-b border-gray-200">
        ...
        <span className="pb-3 border-b-2 border-gray-900 font-medium text-gray-900">{t('nav_compare')}</span>
        <Link href={`/${locale}/trends`} className="pb-3 text-gray-500 hover:text-gray-800">{t('nav_trends')}</Link>
      </div>

      <h1 className="text-3xl font-bold text-gray-900 mb-3">{t('compare_title')}</h1>
      <p className="text-gray-500 mb-2">{t('compare_intro')}</p>
      <p className="text-sm text-gray-400 mb-10">{t('count_articles', { count: compares.length })}</p>
