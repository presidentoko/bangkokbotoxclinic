...
export function BrandCard({ brand, slug, count, locale, modelsLabel, savingsBadge }: Props) {
  return (
    <Link href={`/${locale}/${slug}`} className="group relative overflow-hidden block border border-[#E8E2D9] bg-white hover:border-[#B8954A] hover:shadow-lg transition-all duration-300">
      {/* Top accent bar */}
      ...
      <div className="relative p-6 pb-8">
        ...
        <p className="text-xs text-[#9C8B7A] tracking-wide uppercase mt-1">{count} {modelsLabel}</p>
        {savingsBadge && (
