...
export const OCCASIONS: Occasion[] = [
  ...
  },
  {
    ...
    filterFn: (r) => r.cuisines.some((c) => ["japanese", "street_food", "cafe"].includes(c)) || topicHits(r, "fast_service") >= 1,
    sortBonus: (r) => (r.cuisines.includes("japanese") ? 20 : 0) + (r.cuisines.includes("street_food") ? 15 : 0),
  ...
  },
  {
    ...
    filterFn: (r) => topicHits(r, "affordable") >= 1 || r.cuisines.some((c) => ["street_food", "thai"].includes(c)),
  ...
  },
  {
    ...
    filterFn: (r) => r.cuisines.some((c) => ["bar_pub", "street_food", "thai"].includes(c)) && r.trust_score >= 50,
    // No "late_night" topic exists in the scraped data — bar_pub is the
    // closest real signal for "still open after dark".
    sortBonus: (r) => (r.cuisines.includes("bar_pub") ? 10 : 0),
  ...
  },
  {
    ...
    filterFn: (r) => topicHits(r, "vegetarian_friendly") >= 1 || r.cuisines.some((c) => ["vegetarian", "cafe"].includes(c)) || topicHits(r, "fresh") >= 1,
    sortBonus: (r) => topicHits(r, "vegetarian_friendly") * 8 + topicHits(r, "fresh") * 10,
