...
export default function AgeGroupResults({ focus }: { focus: SiteFocus }) {
  ...
  return (
    <section className="rounded-2xl border bg-white p-5" style={{ borderColor: "rgb(var(--border))" }}>
      <div className="mb-3">
        <div className="text-xs font-black uppercase tracking-widest text-[rgb(var(--muted))]">By age group</div>
        <h3 className="text-base font-black mt-0.5">What to expect in your age range</h3>
      </div>
      <ul className="space-y-2">
        {rows.map((r, i) => (
          <li key={i} className="rounded-xl border bg-slate-50 p-3.5" style={{ borderColor: "rgb(var(--border))" }}>
