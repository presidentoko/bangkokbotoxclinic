...
export default function CertificationsBar() {
  return (
    <section className="rounded-2xl border bg-white px-5 py-4" style={{ borderColor: "rgb(var(--border))" }}>
      <div className="text-[10px] font-black uppercase tracking-widest text-[rgb(var(--muted))] mb-3 text-center">Accreditations across our partner clinics</div>
        ...
        {CERTS.map((c) => (
          <div key={c.code} className="flex items-center gap-2 text-xs">
            ...
            <div>
              ...
              <div className="text-[10px] text-[rgb(var(--muted))]">{c.name}</div>
            </div>
          </div>
        ))}
      </div>
