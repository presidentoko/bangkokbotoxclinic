    ...
    <div className="fixed bottom-20 sm:bottom-6 left-4 sm:left-6 z-30 max-w-[calc(100vw-2rem)] sm:max-w-xs rounded-xl bg-white border-2 border-emerald-200 shadow-xl p-3 toast-fade-up print:hidden">
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[10px] text-[rgb(var(--muted))] mt-0.5">{minsAgo} min{minsAgo === 1 ? "" : "s"} ago · verified</p>
        </div>
        <button
          onClick={() => setActive(false)}
          aria-label="Dismiss"
          className="text-[rgb(var(--muted))] hover:text-black text-xs -m-1 p-1 shrink-0"
        >
          ✕
        </button>
      </div>
