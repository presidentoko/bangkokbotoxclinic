...
export default function AvailabilityHeatmap({ clinicId }: { clinicId: string }) {
  ...
  return (
    <section className="rounded-2xl border bg-white p-5" style={{ borderColor: "rgb(var(--border))" }}>
      <div className="mb-3">
        <div className="text-xs font-black uppercase tracking-widest text-[rgb(var(--muted))]">12-month booking outlook</div>
      ...
      <div className="grid grid-cols-12 gap-1.5">
        {months.map((row, i) => (
          <div key={row.m} className="text-center">
            ...
            <div className="text-[9px] text-[rgb(var(--muted))] mt-1">{row.m}</div>
          </div>
        ))}
      </div>
