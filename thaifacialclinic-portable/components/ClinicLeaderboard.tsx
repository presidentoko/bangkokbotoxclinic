...
export default function ClinicLeaderboard({
  ...
  return (
    <section className="rounded-2xl border bg-white overflow-hidden" style={{ borderColor: "rgb(var(--border))" }}>
      <div className="px-5 py-4 border-b flex items-baseline justify-between gap-3 flex-wrap bg-gradient-to-r from-yellow-50 to-amber-50" style={{ borderColor: "rgb(var(--border))" }}>
        <div>
        ...
        <span className="text-[11px] text-[rgb(var(--muted))]">Ranked by Trust Score — updates weekly</span>
      </div>

      <ol className="divide-y" style={{ borderColor: "rgb(var(--border))" }}>
        {top.map((c, i) => {
          const rank = i + 1;
            ...
            <li key={c.id}>
              <a href={`/clinic/${c.id}`} className="flex items-center gap-4 p-4 hover:bg-slate-50 transition">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-[rgb(var(--muted))] flex items-center gap-2 flex-wrap">
                    {c.city && <span>📍 {c.city}</span>}
                    <span>★ {(c.rating ?? 0).toFixed(1)}</span>
                    <span className="tabular-nums">· {(c.review_count ?? 0).toLocaleString()} reviews</span>
                  </div>
                </div>
                <div className="text-right shrink-0">
                  ...
                  <div className={`text-[10px] font-bold tabular-nums ${
                    delta > 0 ? "text-emerald-700" : delta < 0 ? "text-rose-700" : "text-[rgb(var(--muted))]"
                  }`}>
