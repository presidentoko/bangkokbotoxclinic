  ...
  return (
    <section className="rounded-2xl border bg-white p-5" style={{ borderColor: "rgb(var(--border))" }}>
      <div className="flex items-baseline justify-between mb-1 gap-3 flex-wrap">
        ...
        <span className="text-xs text-[rgb(var(--muted))]">Confirmed by clinic after booking</span>
      </div>
      ...
      <div className="overflow-x-auto -mx-2 px-2 pb-1">
        <table className="min-w-full text-center" style={{ borderSpacing: "4px" }}>
          <thead>
            <tr>
              <th className="text-[10px] font-bold uppercase text-[rgb(var(--muted))] text-left pl-1 py-1">Slot</th>
              {days.map((d, i) => (
                <th key={d.date} className="px-1">
                  <div className={`text-[10px] font-bold uppercase ${i === 0 ? "text-emerald-700" : "text-[rgb(var(--muted))]"}`}>{d.label}</div>
      ...
      <div className="mt-4 flex items-center justify-between gap-2 text-[11px] text-[rgb(var(--muted))] flex-wrap">
