...
export default function ClinicSpinWheel({ clinics, lang = "en" }: { clinics: Clinic[]; lang?: string }) {
  ...
  return (
    <section className="rounded-[2rem] border-2 bg-gradient-to-br from-amber-50 via-yellow-50 to-orange-50 dark:from-amber-950/30 dark:to-orange-950/30 p-6 sm:p-8 text-center" style={{ borderColor: "rgb(var(--border))" }}>
      <div className="mb-5">
      ...
      <div className="mt-5 min-h-[80px]">
        {winner ? (
          <div className="animate-fade-up">
            ...
            <a href={`/${lang}/clinic/${winner.slug}/`}
              className="block mt-1 font-display text-xl font-bold tracking-tighter-display hover:underline">
              {winner.name}
            </a>
            <p className="text-xs muted">{winner.city} · ★ {(winner.rating ?? 0).toFixed(1)} · Trust {winner.trust_score}</p>
