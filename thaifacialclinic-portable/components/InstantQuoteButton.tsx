            ...
            <div className="p-6">
              {sent ? (
                <div className="text-center">
                  ...
                  <p className="text-sm text-[rgb(var(--muted))] mb-4">{clinicName} will email you within 4 hours.</p>
                ...
                <>
                  <button onClick={() => setOpen(false)} aria-label="Close"
                    className="absolute top-3 right-3 text-[rgb(var(--muted))] hover:text-black text-lg -m-2 p-2">✕</button>
                  <div className="text-3xl mb-2">⚡</div>
                  <h3 className="text-xl font-black mb-1">Quick quote from {clinicName}</h3>
                  <p className="text-xs text-[rgb(var(--muted))] mb-4">No phone needed. Reply within 4h.</p>
                  <form onSubmit={submit} className="space-y-2">
                    <input required placeholder="Your name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                      className="w-full rounded-lg border-2 px-3 py-2.5 text-sm outline-none focus:border-amber-500" style={{ borderColor: "rgb(var(--border))" }} />
                    ...
                    <textarea placeholder="Anything specific? (optional)" rows={2} value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })}
                      className="w-full rounded-lg border-2 px-3 py-2.5 text-sm outline-none focus:border-amber-500 resize-none" style={{ borderColor: "rgb(var(--border))" }} />
                    ...
                    <p className="text-[10px] text-center text-[rgb(var(--muted))]">
