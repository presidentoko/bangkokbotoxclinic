  ...
  return (
    <section className="rounded-2xl border bg-white p-5" style={{ borderColor: "rgb(var(--border))" }}>
      <div className="flex items-baseline justify-between gap-3 mb-3 flex-wrap">
        <div>
          <div className="text-xs font-black uppercase tracking-widest text-[rgb(var(--muted))]">Bangkok weather</div>
      ...
      <div className="grid grid-cols-5 gap-2">
        {days.map((d, i) => (
          <div key={i} className="rounded-lg bg-slate-50 border p-2 text-center" style={{ borderColor: "rgb(var(--border))" }}>
            <div className="text-[10px] font-bold uppercase text-[rgb(var(--muted))]">{d.label}</div>
            ...
            <div className="text-[10px] tabular-nums">
              ...
              <span className="text-[rgb(var(--muted))]"> / {d.low}°</span>
            </div>
            <div className="text-[9px] text-[rgb(var(--muted))] mt-0.5">{d.rain}% rain</div>
          </div>
        ))}
      </div>
