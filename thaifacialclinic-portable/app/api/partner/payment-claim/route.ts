...
async function rateLimitOk(ip: string): Promise<boolean> {
  ...
  const key = `ratelimit:payment-claim:${ip}`;
  try {
    const v = (await rcmd(["INCR", key])) as number;
    if (v === 1) await rcmd(["EXPIRE", key, "3600"]);
    return v <= 3;
  } catch {
...
export async function POST(req: Request) {
  ...
  if (!(await rateLimitOk(ip))) return new Response("rate limit", { status: 429 });
