...
export function PriceHistory({ samples, condition = 'very_good' }: Props) {
  ...
  return (
    <section className="border border-[#E8E2D9] rounded-xl p-6 bg-white">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base font-semibold text-[#1A1A1A]">Price History</h2>
            ...
            <div key={month} className="flex-1 flex flex-col items-center gap-1 group">
              <div className="relative w-full">
                <div
                  ...
                  title={`${month}: ${formatPrice(avg)} (${count} listings)`}
                />
              </div>
            </div>
          )
      ...
      <div className="flex justify-between text-xs text-[#8C7355]">
        <span>{months[0]}</span>
        <span>{formatPrice(latestAvg)} avg</span>
        <span>{months[months.length - 1]}</span>
      </div>
      <p className="text-xs text-[#8C7355] mt-2">{filtered.length} listings tracked · Very Good condition</p>
    </section>
  )
}
