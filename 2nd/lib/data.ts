...
export type Category = 'handbags' | 'watches' | 'shoes' | 'jewelry' | 'belts' | 'scarves' | 'small-leather-goods' | 'clothing'
...
export function getItemsUnderBudget(maxUsd: number): Item[] {
  return items.filter(i => {
    const vg = i.price_ranges.very_good
    if (!vg) return false
    return getAvgPrice(vg) <= maxUsd
  })
}
