...
export function makeLeadId(): string {
  // not crypto-strong — just unique enough for non-PK identifier
  return `${Date.now().toString(36)}${Math.random().toString(36).slice(2, 8)}`;
}

export async function storeLead(rec: LeadRecord): Promise<void> {
  const key = `clinic:${rec.clinic_id}:leads`;
  await upstash(["LPUSH", key, JSON.stringify(rec)]);
  await upstash(["LTRIM", key, 0, 199]);            // 최근 200건만 보존
  await upstash(["EXPIRE", key, 60 * 60 * 24 * 90]); // 90일 TTL
  await upstash(["INCR", `clinic:${rec.clinic_id}:lead_count`]);
}
