      ...
      {open && (
        <>
          ...
          <div role="dialog" aria-modal="false"
            className="fixed bottom-20 sm:bottom-6 right-4 sm:right-6 z-40 w-[92vw] max-w-sm rounded-3xl bg-white shadow-2xl overflow-hidden border-2 border-emerald-200 toast-fade-up print:hidden">
            {/* Header */}
              ...
              {msgs.map((m, i) => (
                <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[80%] rounded-2xl px-3 py-2 text-sm leading-relaxed ${
                    m.role === "user" ? "bg-emerald-600 text-white rounded-br-sm" : "bg-white border border-[var(--border)] rounded-bl-sm shadow-sm"
                  }`}>
                    {m.text}
                  </div>
                </div>
            ...
            {msgs.length > 0 && msgs[msgs.length - 1].role === "bot" && (
              <div className="border-t bg-white px-3 py-2 flex flex-wrap gap-1.5" style={{ borderColor: "var(--border)" }}>
                {QUICK_REPLIES.map((q) => (
            ...
            {/* Fallback actions */}
            <div className="border-t bg-white px-3 py-2.5 flex items-center gap-2" style={{ borderColor: "var(--border)" }}>
              ...
              <a href={`mailto:${EMAIL}`}
                className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg bg-white border border-[var(--border)] text-xs font-bold px-3 py-2 hover:bg-slate-50">
                📧 Email
              </a>
            </div>
          </div>
