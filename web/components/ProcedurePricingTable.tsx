...
export default function ProcedurePricingTable({ focus }: { focus: SiteFocus }) {
  ...
  return (
    <section className="rounded-2xl border bg-white p-5" style={{ borderColor: "var(--border)" }}>
      <div className="mb-4">
        <div className="text-xs font-black uppercase tracking-widest text-[var(--muted)]">Transparent pricing</div>
        <h3 className="text-lg sm:text-xl font-black tracking-tight mt-1">All procedures · from-prices</h3>
        <p className="text-xs text-[var(--muted)] mt-1">Bangkok market ranges. Final quote after consult.</p>
      </div>
      <div className="overflow-x-auto -mx-2 px-2">
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-[10px] font-black uppercase tracking-widest text-[var(--muted)] border-b" style={{ borderColor: "var(--border)" }}>
          ...
          <tbody className="divide-y" style={{ borderColor: "var(--border)" }}>
            {rows.map((r, i) => (
              <tr key={i} className="hover:bg-slate-50">
                ...
                <td className="py-2.5 text-xs text-[var(--muted)] hidden sm:table-cell">{r.unit}</td>
