    ...
    <section className="rounded-2xl border-2 bg-gradient-to-br from-emerald-50 via-white to-teal-50 p-5 sm:p-6" style={{ borderColor: "#a7f3d0" }}>
      <div className="mb-4">
        ...
        <p className="mt-1 text-sm text-[var(--muted)]">
          Bangkok baseline: <strong className="text-[var(--fg)]">{fmt(thb)}</strong> ({usd(thb)} USD). All other prices are rough public averages — confirm with each clinic.
        </p>
      </div>
      ...
      <div className="space-y-2.5">
        <div className="flex items-center gap-3 rounded-xl border-2 border-emerald-400 bg-emerald-50 p-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[11px] text-[var(--muted)]">You&apos;re here</div>
          </div>
        ...
        {rows.map((r) => {
          ...
          return (
            <div key={r.flag} className="flex items-center gap-3 rounded-xl bg-white border p-3" style={{ borderColor: "var(--border)" }}>
              <span className="text-2xl shrink-0">{r.flag}</span>
              <div className="flex-1 min-w-0">
                <div className="flex items-baseline justify-between gap-2 mb-1">
                  <div className="font-bold text-sm">{r.name}</div>
                  <div className="text-[10px] text-[var(--muted)]">{r.note}</div>
                </div>
