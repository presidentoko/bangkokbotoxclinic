...
export default function TravelPackageWidget({ procedureCostTHB }: { procedureCostTHB?: number }) {
  return (
    <section className="rounded-2xl border bg-white p-5 sm:p-6" style={{ borderColor: "var(--border)" }}>
      <div className="mb-4">
        <div className="text-xs font-black uppercase tracking-widest text-[var(--muted)]">All-in trip cost</div>
        <h3 className="text-lg sm:text-xl font-black tracking-tight mt-1">Even with travel, Bangkok wins</h3>
        <p className="text-xs text-[var(--muted)] mt-1">Roundtrip flight + 5-night decent hotel. Off-peak averages, 2-week advance booking.</p>
      </div>
      ...
      <ul className="grid gap-2 sm:grid-cols-2">
        {FLIGHTS.map((f) => {
          ...
          return (
            <li key={f.city} className="rounded-lg border bg-slate-50 p-3 flex items-center gap-3" style={{ borderColor: "var(--border)" }}>
