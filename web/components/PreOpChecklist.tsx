...
export default function PreOpChecklist({ focus }: { focus: SiteFocus }) {
  ...
  return (
    <section className="rounded-2xl border bg-white p-5 sm:p-6" style={{ borderColor: "var(--border)" }}>
      <div className="mb-4">
        <div className="text-xs font-black uppercase tracking-widest text-[var(--muted)]">Prep guide</div>
        <h3 className="text-lg sm:text-xl font-black tracking-tight mt-1">Before your procedure</h3>
      </div>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {list.map((s, i) => (
          <div key={i} className="rounded-xl border bg-slate-50 p-4" style={{ borderColor: "var(--border)" }}>
