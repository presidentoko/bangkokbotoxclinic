...
export default function MultiCityCompare({
  ...
  return (
    <section className="rounded-2xl border bg-white p-5" style={{ borderColor: "var(--border)" }}>
      <div className="mb-3">
        <div className="text-xs font-black uppercase tracking-widest text-[var(--muted)]">Thailand by city</div>
      ...
      <ul className="grid gap-2 sm:grid-cols-2">
        {rows.map((r) => (
          <li key={r.k} className={`rounded-lg p-3 border-2 flex items-center gap-3 ${
            r.k === highlight ? "border-emerald-500 bg-emerald-50" : "border-[var(--border)] bg-slate-50"
          }`}>
            ...
            <div className="text-right shrink-0">
              ...
              <div className="text-[10px] text-[var(--muted)]">{r.mult === 1 ? "baseline" : r.mult > 1 ? `+${((r.mult - 1) * 100).toFixed(0)}%` : `${((r.mult - 1) * 100).toFixed(0)}%`}</div>
            </div>
          </li>
        ))}
      </ul>
