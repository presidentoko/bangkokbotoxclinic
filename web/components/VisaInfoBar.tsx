  ...
  return (
    <section className="rounded-2xl border bg-white p-5" style={{ borderColor: "var(--border)" }}>
      <div className="flex items-baseline justify-between gap-3 mb-3 flex-wrap">
        <div>
          <div className="text-xs font-black uppercase tracking-widest text-[var(--muted)]">Visa quick check</div>
      ...
      {open ? (
          ...
          {RULES.map((r) => (
            <button key={r.country} onClick={() => { setCountry(r); setOpen(false); }}
              className={`flex items-center gap-2 rounded-lg border-2 px-3 py-2 text-xs font-bold text-left transition ${
                country.country === r.country ? "border-emerald-500 bg-emerald-50" : "border-[var(--border)] hover:border-slate-400"
              }`}>
        ...
        <div className="grid sm:grid-cols-3 gap-3">
          <div className="rounded-lg border bg-slate-50 p-3" style={{ borderColor: "var(--border)" }}>
            <div className="text-[10px] font-bold uppercase tracking-wider text-[var(--muted)]">Visa</div>
            <div className="text-sm font-black mt-0.5">{country.visa}</div>
          </div>
          <div className="rounded-lg border bg-slate-50 p-3" style={{ borderColor: "var(--border)" }}>
            <div className="text-[10px] font-bold uppercase tracking-wider text-[var(--muted)]">Max stay</div>
            <div className="text-sm font-black mt-0.5">{country.days}</div>
          </div>
          <div className="rounded-lg border bg-slate-50 p-3" style={{ borderColor: "var(--border)" }}>
            <div className="text-[10px] font-bold uppercase tracking-wider text-[var(--muted)]">Note</div>
