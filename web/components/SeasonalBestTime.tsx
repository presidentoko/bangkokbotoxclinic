...
export default function SeasonalBestTime({ focus }: { focus: SiteFocus }) {
  ...
  return (
    <section className="rounded-2xl border bg-white p-5" style={{ borderColor: "var(--border)" }}>
      <div className="mb-3">
        <div className="text-xs font-black uppercase tracking-widest text-[var(--muted)]">When to come</div>
        ...
        {MONTHS.map((m, i) => {
          ...
          return (
            <div key={m} className="text-center">
              ...
              <div className="text-[9px] text-[var(--muted)] mt-1">{m}</div>
            </div>
          );
        })}
      </div>
      ...
      <p className="text-[11px] text-[var(--muted)] mt-3 leading-relaxed">{cfg.legend}</p>
    </section>
  );
}
