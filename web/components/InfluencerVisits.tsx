  ...
  return (
    <section className="rounded-2xl border bg-white p-5" style={{ borderColor: "var(--border)" }}>
      <div className="flex items-baseline justify-between gap-3 mb-3 flex-wrap">
        ...
        <span className="text-xs text-[var(--muted)]">Search before you book — they did</span>
      </div>
      <div className="grid gap-2 sm:grid-cols-2">
        {list.map((inf, i) => (
          <div key={i} className="flex items-center gap-3 rounded-lg border bg-slate-50 p-3" style={{ borderColor: "var(--border)" }}>
            <span className="text-2xl shrink-0">{inf.flag}</span>
            <div className="flex-1 min-w-0">
              <div className="font-bold text-sm truncate">{inf.handle}</div>
              <div className="text-[11px] text-[var(--muted)] truncate">{inf.topic}</div>
            </div>
