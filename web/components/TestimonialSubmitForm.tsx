  ...
  return (
    <section className="rounded-2xl border bg-white p-5" style={{ borderColor: "var(--border)" }}>
      <div className="flex items-baseline justify-between gap-3 mb-3 flex-wrap">
        <div>
          <div className="text-xs font-black uppercase tracking-widest text-[var(--muted)]">Community</div>
      ...
      {sent ? (
        ...
        <form onSubmit={submit} className="space-y-2">
          ...
          <input placeholder="Procedure (e.g. FUE 2,500 grafts)" value={f.procedure} onChange={(e) => setF({ ...f, procedure: e.target.value })}
            className="w-full rounded-lg border-2 px-3 py-2.5 text-sm outline-none focus:border-emerald-500" style={{ borderColor: "var(--border)" }} />
          <textarea required rows={4} placeholder="What happened? What worked, what didn't. Honest helps others." value={f.story} onChange={(e) => setF({ ...f, story: e.target.value })}
            className="w-full rounded-lg border-2 px-3 py-2.5 text-sm outline-none focus:border-emerald-500 resize-none" style={{ borderColor: "var(--border)" }} />
          <div className="flex items-center justify-between gap-2">
            <div className="flex gap-1 items-center text-sm">
              <span className="text-[var(--muted)] text-xs mr-1">Rating:</span>
              {[1,2,3,4,5].map((n) => (
          ...
          <p className="text-[10px] text-[var(--muted)] mt-1">We publish helpful stories anonymously. We never share email.</p>
        </form>
      ) : (
