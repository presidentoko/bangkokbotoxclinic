  ...
  return (
    <section>
      <div className="flex items-baseline justify-between mb-4 gap-3">
        <div>
          <div className="text-xs font-black uppercase tracking-widest text-[var(--muted)]">Patient stories</div>
        ...
        <div className="flex gap-2 shrink-0">
          <button onClick={() => scroll(-1)} aria-label="Scroll left"
            className="grid h-9 w-9 place-items-center rounded-full border bg-white hover:bg-slate-50" style={{ borderColor: "var(--border)" }}>‹</button>
      ...
      <div ref={ref} className="flex snap-x snap-mandatory gap-4 overflow-x-auto pb-3 -mx-4 px-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {stories.map((s, i) => (
          <article key={i} className="w-[300px] sm:w-[340px] shrink-0 snap-start rounded-2xl border bg-white p-5 shadow-sm" style={{ borderColor: "var(--border)" }}>
            <div className="flex items-center gap-3 mb-3">
              <span className="text-3xl shrink-0">{s.flag}</span>
              <div className="min-w-0">
                <div className="font-black text-sm">{s.name}, {s.age}</div>
                <div className="text-[11px] text-[var(--muted)]">{s.from} · {s.daysAgo < 60 ? `${s.daysAgo}d ago` : s.daysAgo < 365 ? `${Math.round(s.daysAgo / 30)}mo ago` : "1yr+ ago"}</div>
              </div>
            </div>
