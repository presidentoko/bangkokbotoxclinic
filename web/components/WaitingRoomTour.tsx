  ...
  return (
    <section className="rounded-2xl border bg-white p-5" style={{ borderColor: "var(--border)" }}>
      <div className="flex items-baseline justify-between mb-3 gap-3 flex-wrap">
        <div>
          <div className="text-xs font-black uppercase tracking-widest text-[var(--muted)]">Inside the clinic</div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {items.map((it, i) => (
          <div key={i} className="rounded-xl border bg-slate-50 p-3 text-center" style={{ borderColor: "var(--border)" }}>
            <div className="text-2xl mb-1">{it.emoji}</div>
            <div className="font-black text-xs">{it.title}</div>
            <p className="text-[10px] text-[var(--muted)] mt-1 leading-snug">{it.body}</p>
          </div>
        ))}
      </div>
    </section>
